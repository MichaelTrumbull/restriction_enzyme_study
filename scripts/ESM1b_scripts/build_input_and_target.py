...
BUILD_motif1stHalf2ndHalf_numNbase10 = True
BUILD_motif1stHalf2ndHalf_ordered_numNbase10 = True
...
############################################################################################################################################
# Same idea as BUILD_Motif_1stHalf_Motif_2ndHalf_numN except: 
# I will use the value of the numer of N (instead of onehot), remove N from any sequence, pad left/right to a consistent number (probably 5)
# ##########################################################################################################################################

motifs_pd = pd.read_csv("raw_data/metadata_600.csv")
df = motifs_pd
if BUILD_motif1stHalf2ndHalf_numNbase10:
    # set up directory of how each symbol connects to ACGT: http://www.hgmd.cf.ac.uk/docs/nuc_lett.html
    # mapping 'N': [1,1,1,1] because it should accept any base target. Could be [0,0,0,0] because this is just padding...
    bas4_dict = {
        'A': [1,0,0,0], 'C': [0,1,0,0], 'G': [0,0,1,0], 'T': [0,0,0,1],
        'R': [1,0,1,0], 'Y': [0,1,0,1], 'K': [0,0,1,1], 'M': [1,1,0,0], 'S': [0,1,1,0], 'W': [1,0,0,1],
        'B': [0,1,1,1], 'D': [1,0,1,1], 'H': [1,1,0,1], 'V': [1,1,1,0]#, 'N': [1,1,1,1] #This N value should not be used in this script
    }
    ...
    def set_pat(data, pad_size):
        # data: list of onehot-coded sequences
        # pad_size: what size to pad to for each seq
        pass ############################################ PUT A METHOD HERE!
