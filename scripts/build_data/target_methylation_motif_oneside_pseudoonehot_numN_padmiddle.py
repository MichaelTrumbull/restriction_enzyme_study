...
bas4_dict = {
    ...
    'R': [1,0,1,0], 'Y': [0,1,0,1], 'K': [0,0,1,1], 'M': [1,1,0,0], 
    'S': [0,1,1,0], 'W': [1,0,0,1],'B': [0,1,1,1], 'D': [1,0,1,1], 
    'H': [1,1,0,1], 'V': [1,1,1,0], 'N': [1,1,1,1], '0': [0,0,0,0]
}
...
pad_len = 6 # determined manually is 5. So pad the longest one makes 6
seq_start_pad = [item + '0'*(pad_len-len(item)) for item in seq_start] # now we padded the first half to 6

seq_end = [item[6:].replace('N','') for item in left_seqs]
seq_end_pad = [item + '0'*(pad_len-len(item)) for item in seq_end]

left_seqs = [a + b for a,b in zip(seq_start_pad, seq_end_pad)]# now it is the same as before but we removed all the Ns and gave padding accordingly with 0s
#############
onehot_seqs = []
...
##### new code
[a.append(b) for a,b in zip(onehot_seqs, num_N_list)] # put the number N at the end
...
#
#torch.save(torch.FloatTensor(padded_onehot_seqs), "data/target_methylation_motif_oneside_trueonehot.pt")
torch.save(torch.FloatTensor(onehot_seqs), "data/target_methylation_motif_oneside_pseudoonehot_numN_padmiddle.pt")
# 49 len
